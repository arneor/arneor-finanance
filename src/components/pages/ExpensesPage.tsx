...
export default function ExpensesPage() {
  ...
  const expenseTransactions = useMemo(() => transactions.filter((t) => t.Type === 'Expense' && t.Transaction_ID).sort((a, b) => new Date(b.Date).getTime() - new Date(a.Date).getTime()), [transactions]);
  ...
  const handleAdd = async () => {
    ...
    setSubmitting(true);
    try {
      await addTransaction({ Date: form.Date, Type: 'Expense', Category: form.Category, Amount: parseFloat(form.Amount), Partner_Account: form.Partner_Account, Description: form.Description, Payment_Method: form.Payment_Method, Tags: form.Tags, Added_By: form.Added_By });
      addAlert({ type: 'success', title: 'Added', message: 'Expense recorded' });
      setShowAdd(false);
  ...
  return (
    <div className="page-content">
      <div className="page-header-bar">
        <div><h2 className="page-subtitle">Total Operating Expenses</h2><p className="page-big-value amount-negative">{formatCurrency(totalExpenses)}</p></div>
        <button className="btn btn-primary" onClick={() => setShowAdd(true)}><Plus size={16} /> Record Expense</button>
      </div>

      <div className="charts-grid grid-2">
        <Card title="Expense Trend" className="chart-card">
          <div className="chart-container">
            <Line data={{ labels: monthlyData.labels, datasets: [{ label: 'Expenses', data: monthlyData.expenses, borderColor: CHART_COLORS.danger, backgroundColor: 'rgba(239,68,68,0.1)', fill: true, tension: 0.4 }] }} options={chartOpts} />
          </div>
        </Card>
              ...
              {expenseTransactions.length === 0 ? <tr><td colSpan={7} className="no-data-cell">No expenses recorded</td></tr> :
                expenseTransactions.slice(0, 50).map((t) => (
                  <tr key={t.Transaction_ID}>
                    ...
                    <td>{t.Category}</td>
                    <td className="amount-negative"><ArrowDownRight size={14} /> {formatCurrency(t.Amount)}</td>
      ...
      <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="Add Expense" size="lg">
